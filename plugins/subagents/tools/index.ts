...
async function runSingleAgent(
   ...
   try {
      ...
      return await new Promise<SingleResult>(resolve => {
         ...
         const ensureNonEmptyText = (text: string, exitCode: number) => {
            if (text.trim()) return text

            const lines = [
               'Status: ' + (exitCode === 0 ? 'success' : 'blocked') + ' - ' + agentName + ' completed without final text output',
            ]
            ...
            if (recentTools.length > 0) {
               lines.push('Recent tools: ' + recentTools.map(t => (t.desc ? t.tool + '(' + t.desc + ')' : t.tool)).join(', '))
            }
            ...
            if (stderrContent.trim()) {
               lines.push('Stderr:')
               lines.push(stderrContent.trim().slice(0, 2000))
            }
         ...
         rl.on('line', line => {
            try {
               ...
               if (event.type === 'tool_execution_start') {
                  ...
                  const stdoutContent = ensureNonEmptyText(lastTextContent, 0)
                  const stdoutResult = truncateOutput(stdoutContent)
                  doResolve({
                     agent: agentName,
                     agentSource: agent.source,
                     task,
         ...
         proc.on('close', code => {
            ...
            const stdoutContent = ensureNonEmptyText(lastTextContent, code ?? 0)
            const stdoutResult = truncateOutput(stdoutContent)
